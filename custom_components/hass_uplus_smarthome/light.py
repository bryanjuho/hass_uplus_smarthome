...
class UplusMoodLight(LightEntity):
    ...
    async def async_update(self):
        ...
        header = {
            ...
            'Referer': 'https://hmiotsas.uplus.co.kr:7002/smartlight/smartlight/main.html'
                       '?mid=LC100&uuid=93BAB3FB-292B-34AC-93E1-70DDD599181C&pmid=HS001&vendor=0000584&model=LT-Z24C01',
            'X-HIT-SvcCode': '9119',
            'Content-Length': '411',
        ...
        params = ['device_state', 'brightness', 'color']
        status_to_dict = {k: v for status in statuses for k, v in status.items() if k in params}

        self.api.light.is_on = bool(status_to_dict['device_state'])
        self.api.light.brightness = status_to_dict['brightness']
